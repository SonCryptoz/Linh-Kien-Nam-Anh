            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="row">
                        ...
                        if (isset($_GET['price_from'])) {
                            switch ($_GET['price_from']) {
                                case 1:
                                    $conditions[] = "price <= 1000000";
                                    break;
                                case 2:
                                    $conditions[] = "price > 1000000 AND price <= 10000000";
                                    break;
                                case 3:
                                    $conditions[] = "price > 10000000 AND price <= 50000000";
                                    break;
                                default:
                                    $conditions[] = "price > 50000000";
                            }
                        }
                        ...
                        while ($row = $rs->fetch_array(MYSQLI_ASSOC)) {
                            if ($count_product >= ($page_number - 1) * $numProductInAPage && $count_product < ($page_number - 1) * $numProductInAPage + $numProductInAPage) {
                                ...
                                echo "<a href='delete-product.php?id={$row['product_id']}' onclick='return confirm(\"Bạn có chắc chắn muốn xóa sản phẩm này không?\");'><i class='far fa-trash-alt'></i></a>";
                                ...
                                echo "<h2 style='line-height: 20px; max-height: 40px; overflow: hidden; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2;'>{$row['product_name']}</h2>";
                                if ($row['discount'] != 0) {
                    ...
                    <div class="col-md-12">
                        <nav aria-label="Page navigation example">
                            <ul class="pagination justify-content-center">
                                <?php
                                // Khởi tạo các tham số từ URL
                                $sort = $_GET['sort'] ?? '';
                                $price_from = $_GET['price_from'] ?? '';
                                $search = $_GET['search'] ?? '';

                                $totalProduct = mysqli_num_rows($rs);
                                $totalPage = ceil($totalProduct / $numProductInAPage); // Tính tổng số trang
                                ...
                                for ($count = 1; $count <= $totalPage; $count++) {
                                    ...
                                    if (!empty($price_from)) {
                                        $link .= "&price_from=" . urlencode($price_from);
                                    }
                                    if (!empty($search)) {
                                        $link .= "&search=" . urlencode($search);
                                    }

                                    $link = htmlspecialchars($link); // Bảo mật XSS
